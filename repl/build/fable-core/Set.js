define(["require", "exports", "./Comparer", "./List", "./List", "./Seq", "./Seq", "./Seq", "./Seq", "./Seq", "./Seq", "./Seq", "./Symbol", "./Util"], function (require, exports, Comparer_1, List_1, List_2, Seq_1, Seq_2, Seq_3, Seq_4, Seq_5, Seq_6, Seq_7, Symbol_1, Util_1) {
    "use strict";
    ...
    function distinctBy(f, xs) {
        return Seq_5.choose((tup) => tup[0], Seq_4.scan((tup, x) => {
            const acc = tup[1];
            const k = f(x);
    ...
    function unionInPlace(set1, set2) {
        Seq_6.iterate((x) => { set1.add(x); }, set2);
    }
    ...
    function unionMany(sets) {
        ...
        return Seq_7.fold((acc, s) => union(s, acc), create(), sets);
    }
    ...
    function differenceInPlace(set1, set2) {
        Seq_6.iterate((x) => { set1.delete(x); }, set2);
    }
    ...
    function intersectInPlace(set1, set2) {
        ...
        Seq_6.iterate((x) => { if (!set2_.has(x)) {
            set1.delete(x);
        } }, set1);
    }
    ...
    function isProperSubsetOf(set1, set2) {
        if (set1 instanceof FableSet && set2 instanceof FableSet) {
            ...
            return Seq_2.forAll((x) => set2.has(x), set1) && Seq_1.exists((x) => !set1.has(x), set2);
        }
    }
    ...
    function isSubsetOf(set1, set2) {
        if (set1 instanceof FableSet && set2 instanceof FableSet) {
            ...
            set2 = set2 instanceof Set ? set2 : new Set(set2);
            return Seq_2.forAll((x) => set2.has(x), set1);
        }
    }
