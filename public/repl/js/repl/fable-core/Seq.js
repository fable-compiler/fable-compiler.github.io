(function (factory) {
    if (typeof module === "object" && typeof module.exports === "object") {
        ...
        define(["require", "exports", "./Array", "./Array", "./ListClass", "./Option", "./Util"], factory);
    }
    ...
    function tryHead(xs) {
        const iter = xs[Symbol.iterator]();
        const cur = iter.next();
        return cur.done ? null : Option_1.makeSome(cur.value);
    }
    ...
    function tryItem(i, xs) {
        ...
        if (Array.isArray(xs) || ArrayBuffer.isView(xs)) {
            return i < xs.length ? Option_1.makeSome(xs[i]) : null;
        }
        for (let j = 0, iter = xs[Symbol.iterator]();; j++) {
            ...
            if (j === i) {
                return Option_1.makeSome(cur.value);
            }
        }
        return null;
    }
    ...
    function tryLast(xs) {
        try {
            return Option_1.makeSome(reduce((_, x) => x, xs));
        }
        catch (err) {
            return null;
        }
    ...
    function tryFind(f, xs, defaultValue) {
        ...
        return defaultValue === void 0 ? null : Option_1.makeSome(defaultValue);
    }
